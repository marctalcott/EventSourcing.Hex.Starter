...
namespace Utility.CosmosValidator
{
    public class CosmosValidatorEngine : IValidatorEngine
    {
        ...
        private CosmosDBEventStore GetMigratedEventStore()
        {
            var eventStore =
                new CosmosDBEventStore(_eventTypeResolver, _endpointUrl, _authorizationKey, _databaseId,
                    _migratedEventContainerId);
            return eventStore;
        }

        private CosmosDBEventStore GetValidationSuccessEventStore()
        {
            var eventStore =
                new CosmosDBEventStore(_eventTypeResolver, _endpointUrl, _authorizationKey, _databaseId,
        ...
        private CosmosDBEventStore GetValidationFailureEventStore()
        {
            var eventStore =
                new CosmosDBEventStore(_eventTypeResolver, _endpointUrl, _authorizationKey, _databaseId,
        ...
        private async Task HandleChangesAsync(IReadOnlyCollection<Change> changes, CancellationToken cancellationToken)
        {
            ...
            foreach (var change in changes)
            {
                ...
                {
                    saved = await failureEventStore.AppendToStreamAsync(
                        change.UserInfo,
                ...
                {
                    saved = await successEventStore.AppendToStreamAsync(
                        change.UserInfo,
                        change.StreamInfo.Id,
                        change.StreamInfo.Version - 1,
                        new[] { completedEvent });
